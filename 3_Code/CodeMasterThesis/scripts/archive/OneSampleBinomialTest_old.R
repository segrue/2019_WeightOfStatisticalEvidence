...
for (n_study in n_studies) {
  ...
  T_clt <- rbind(
    dat_transform(T_clt_emp_avg, T_clt_emp_sd, "emp", "clt", n_study, cols_evidence, p0s, p1s),
  ...
  T_vst <- rbind(
    dat_transform(T_vst_emp_avg, T_vst_emp_sd, "emp", "vst", n_study, cols_evidence, p0s, p1s),
  ...
  for (alph in alphas) {
    ...
    emp_ci_clt <- T_averager(ci_coverage(T_clt_emp, T_clt_th_avg, crit_val_ci), p0s, p1s)
    emp_ci_vst <- T_averager(ci_coverage(T_vst_emp, T_vst_th_avg, crit_val_ci), p0s, p1s)
