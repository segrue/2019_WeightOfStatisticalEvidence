...
for (n_study in n_studies) {
  ...
  Zn_emp <- calc_T(mu0s,mu1_hats,sgm_th,n_study,vst_var_known)
  Zn_emp_avg <- T_averager(Zn_emp,mu0s,mu1s)
  Zn_emp_sd <- T_sd(Zn_emp,mu0s,mu1s)
  
  Zn_th_avg <- sapply(mu1s,function(mu1) vst_var_known(mu0s,mu1,sgm0,n_study))
  T_th_sd <- matrix(1,nrow=dim(Zn_th_avg)[1],ncol=dim(Zn_th_avg)[2])
  
  Zn <- rbind(dat_transform(Zn_emp_avg,Zn_emp_sd,"emp","clt",n_study,cols_evidence,mu0s,mu1s),
                 dat_transform(Zn_th_avg,T_th_sd,"th","clt",n_study,cols_evidence,mu0s,mu1s))
  
  #T_avg_clt <- dat_transform_th_emp(list(Zn_th_avg,Zn_emp_avg))
  ...
  T_clt_stud <-  rbind(dat_transform(T_clt_emp_stud_avg,T_clt_emp_stud_sd,"emp","stud",n_study,cols_evidence,mu0s,mu1s),
                       dat_transform(T_clt_th_avg,T_th_sd,"th","stud",n_study,cols_evidence,mu0s,mu1s))
  ...
  for (alph in alphas){
    ...
    emp_ci_clt <- T_averager(ci_coverage(T_clt_emp,T_clt_th_avg,crit_val_ci),mu0s,mu1s)
    emp_ci_clt_stud <- T_averager(ci_coverage(T_clt_emp_stud,T_clt_th_avg,crit_val_ci_stud),mu0s,mu1s)
    emp_ci_vst <- T_averager(ci_coverage(T_vst_emp,T_vst_th_avg,crit_val_ci),mu0s,mu1s)
...
save(evidence_mean, file = paste0("data/",name,".RData")) #save data so that it doesn't have to be computed again and again


### plot data (to be transferred to a different script) ----
