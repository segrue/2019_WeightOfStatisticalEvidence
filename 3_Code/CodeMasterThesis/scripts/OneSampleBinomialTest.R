...
for (n_study in n_studies) {
  ...
  Vn_emp_avg <- avg_evidence(Vn_emp, p0s, p1s)
  Vn_emp_sd <- sd_evidence(Vn_emp, p0s, p1s)

  Vn_th_avg <- sapply(p1s, function(p1) vst_binom(p0s, p1, n_study))
  Vn_th_sd <- Zn_th_sd
  ...
  for (alph in alphas) {
    ...
    crit_vals_binom <- sapply(p0s, function(p0) qbinom(alph, n_study, 
                                                       p0, lower.tail = FALSE))
    pows_avg_binom_th <- sapply(1:dim(p1s_th)[2], 
                                function(i) apply(sapply(p1s_th[, i], 
                                                         function(p1) 1 - pbinom(crit_vals_binom, 
                                                                                 n_study, p1, lower.tail = TRUE)), 1, mean))
    ...
    pows_avg_Zn_th <- sapply(
      ...
      function(i) sapply(
          p0s,
          function(p0) mean(sapply(
              p1s_th[, i],
              function(p1) 1 - pnorm(crit_val_norm, z_stat_binom(p0, p1, n_study), sd = 1)
            ))
        )
    )
    pows_avg_Zn_emp <- avg_evidence(1 * (Zn_emp > crit_val_norm), p0s, p1s)
    ...
    pows_avg_Vn_th <- sapply(
      ...
      function(i) sapply(
          ...
          function(p0) mean(sapply(
              p1s_th[, i],
              function(p1) 1 - pnorm(crit_val_norm, vst_binom(p0, p1, n_study), sd = 1)
            ))
        )
    )
    pows_avg_Vn_emp <- avg_evidence(1 * (Vn_emp > crit_val_norm), p0s, p1s)
