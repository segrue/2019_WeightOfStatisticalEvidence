...
\begin{itemize}
  \item \texttt{all\_tweets}: the whole set of rated tweets (\Sexpr{l(ntweets[1])}~tweets)
  \item \texttt{one\_hundred}: the rated tweets of those users who sent at least 100 tweets (\Sexpr{l(ntweets[2])}~tweets)
  \item \texttt{sick\_users}: the rated tweets of all those users who sent at least one sick tweet (\Sexpr{l(ntweets[3])}~tweets)
\end{itemize}
}
...
\frametitle{The \texttt{sick\_users} data set - full}
  ...
  \caption{\Sexpr{l(ntweets[3])}~tweets from 222'446 users before pre-processing}
  \end{figure}
}
...
\frametitle{\texttt{sick\_users}: pruned tweets}
  ...
  \caption{\Sexpr{l(df_summary$num_all_tweets)} tweets from \Sexpr{l(df_summary$num_all_users)} users after pre-processing}
  \end{figure}
}
...
\caption{Mean = \Sexpr{r2(avg_med_both[[1]])} (solid line); median = \Sexpr{avg_med_both[[2]]} (dashed line)}
\end{figure}
}
...
\caption{Mean = \Sexpr{r2(avg_med_healthy[[1]])} (solid line); median = \Sexpr{avg_med_healthy[[2]]} (dashed line)}
\end{figure}
}
...
\centering
  ...
  \caption{\Sexpr{l(ntweets[1])} tweets sent by \Sexpr{l(num_user_before)} users before pruning; \Sexpr{l(ntweets_full_pruned)} tweets and \Sexpr{l(num_user_after)} users after pruning}
  \end{figure}
}
...
\frametitle{\texttt{all\_tweets}: sick tweets over time}
  ...
  \caption{\Sexpr{l(sum(df_processed$sick))} sick tweets from max. \Sexpr{l(tot_sick_user)} users}
  \end{figure}
}
...
& \Sexpr{r2(gammas_cdc_25[1])} & \Sexpr{r2(betas_cdc_25[1])}  & \Sexpr{format(errors_cdc_25[1],digits=2)}   \\ 
 {\multirow{-2}{*}{ 2011-2012 (Bod)}}  & \cellcolor{grey}\Sexpr{r2(gammas_full_25[1])}  & \cellcolor{grey}\Sexpr{r2(betas_full_25[1])} & \cellcolor{grey}\Sexpr{format(errors_full_25[1],digits=2)}  \\ \cline{2-4}
 \pause
